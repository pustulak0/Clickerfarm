...
let nightIncomeQuestCount = 0;

let nightIncomeQuestCompleted = false;
...
let tonBonus =
    Number(localStorage.getItem("tonBonus")) || 0;
...
let riceGrowUpgrades = 0;

let potatoGrowUpgrades = 0;

let cornGrowUpgrades = 0;
...
let riceSoupQuestCount = 0;

let riceSoupQuestCompleted = false;
...
let energyUpgradeCost = 5000;

let farmerCost =
    Number(localStorage.getItem("farmerCost")) || 100;

let luckyCoinMultiplier = 0;

let luckyCoinMultiplierCost = 5000;
...
if (
   ...
   let coins1MAchievementUnlocked = false;

let coins10MAchievementUnlocked = false;

let coins25MAchievementUnlocked = false;

let coins75MAchievementUnlocked = false;

let coins225MAchievementUnlocked = false;

let coins750MAchievementUnlocked = false;

let coins1BAchievementUnlocked = false;

let coins5BAchievementUnlocked = false;

let coins10BAchievementUnlocked = false;

let cpsBonus = 0;

let cps100AchievementUnlocked = false;

let cps500AchievementUnlocked = false;

let cps800AchievementUnlocked = false;

let cps1250AchievementUnlocked = false;

let cps1750AchievementUnlocked = false;

let cps2500AchievementUnlocked = false;

let cps3500AchievementUnlocked = false;

let cps4500AchievementUnlocked = false;

let cps6000AchievementUnlocked = false;

let clicks1000AchievementUnlocked = false;

let clicks5000AchievementUnlocked = false;

let clicks12500AchievementUnlocked = false;

let clicks25000AchievementUnlocked = false;

let clicks50000AchievementUnlocked = false;

let clicks80000AchievementUnlocked = false;

let clicks125000AchievementUnlocked = false;

let clicks180000AchievementUnlocked = false;

let clicks250000AchievementUnlocked = false;

let gems100AchievementUnlocked = false;

let gems250AchievementUnlocked = false;

let gems500AchievementUnlocked = false;

let gems1000AchievementUnlocked = false;

let gems1750AchievementUnlocked = false;

let gems3000AchievementUnlocked = false;

let gems5000AchievementUnlocked = false;

let gems7500AchievementUnlocked = false;

let gems11000AchievementUnlocked = false;
 ...
 function checkGemAchievements() {
     ...
     // Gem achievements are handled by the server.
     // The frontend only updates the visual achievement display.
...
async function checkCoinAchievements() {
    ...
    if (!telegramInitData) {
        ...
        console.log(
            "COIN ACHIEVEMENTS: Telegram data not ready."
        );
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/coin-achievements",
                {
                    method: "POST",
        ...
        if (!response.ok) {
            ...
            console.error(
                "COIN ACHIEVEMENTS SERVER ERROR:",
                data
            );
    ...
    Number(
        ...
        console.log(
            "SERVER COIN ACHIEVEMENTS UPDATED:",
            data
        );
...
let farmers50AchievementUnlocked = false;

let farmers125AchievementUnlocked = false;

let farmers180AchievementUnlocked = false;

let farmers275AchievementUnlocked = false;

let farmers400AchievementUnlocked = false;

let farmers600AchievementUnlocked = false;

let farmers800AchievementUnlocked = false;

let farmers950AchievementUnlocked = false;

let farmers1100AchievementUnlocked = false;
...
/* =========================
   COIN ACHIEVEMENT BONUS
========================= */

let coinAchievementBonus = 0;
...
/* =========================================================
   NORMAL UPGRADE FUNCTION - SERVER SIDE
========================================================= */
...
async function performUpgrade() {
    ...
    try {
        ...
        const response = await fetch(
            "https://server-72ja.onrender.com/api/upgrade",
            {
                method: "POST",
    ...
    energyPlus.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/use-energy-refill",
                        {
                            method: "POST",
...
function updateDisplay() {
    ...
    if (coinsPerSecondElement) {
        ...
        coinsPerSecondElement.textContent =
            Number(actualPerSecond.toFixed(1));
    }
...
const inventoryPotato =
    document.getElementById("inventoryPotato");
...
const totalTapPower =
    tapPower *
    (1 + Number(tapBonus || 0) / 100);

if (powerDisplay) {
    powerDisplay.textContent =
        formatNumber(Math.floor(totalTapPower));
}
...
cornBtn.addEventListener("click", async function () {
...
const totalTapPower = Math.floor(
    tapPower * (1 + Number(tapBonus || 0) / 100)
);
...
try {
    ...
    const response =
        await fetch(
            "https://server-72ja.onrender.com/api/tap",
            {
                method: "POST",
    ...
    console.log(
        "SERVER MILESTONES COMPLETED:",
        milestonesCompleted
    );


    /* =====================================================
       CLICK ACHIEVEMENT FLAGS
    ===================================================== */

    clicks1000AchievementUnlocked =
        Boolean(
            data.clicks1000Achievement
        );

    clicks5000AchievementUnlocked =
        Boolean(
            data.clicks5000Achievement
        );

    clicks12500AchievementUnlocked =
        Boolean(
            data.clicks12500Achievement
        );

    clicks25000AchievementUnlocked =
        Boolean(
            data.clicks25000Achievement
        );

    clicks50000AchievementUnlocked =
        Boolean(
            data.clicks50000Achievement
        );

    clicks80000AchievementUnlocked =
        Boolean(
            data.clicks80000Achievement
        );

    clicks125000AchievementUnlocked =
        Boolean(
            data.clicks125000Achievement
        );

    clicks180000AchievementUnlocked =
        Boolean(
            data.clicks180000Achievement
        );

    clicks250000AchievementUnlocked =
        Boolean(
            data.clicks250000Achievement
        );


    /* =====================================================
       SHOW NEW CLICK ACHIEVEMENTS
    ===================================================== */
...
/*
   IMPORTANT:
   totalClicks is now server-authoritative.
   DO NOT do totalClicks++ here.
*/
...
minerBtn.addEventListener(
    ...
    async function () {
        ...
        try {
            ...
            const response =
                await fetch(
                    "https://server-72ja.onrender.com/api/buy-basic-farmer",
                    {
                        method: "POST",
            ...
            farmerCost =
                Number(data.farmerCost);


            /* =====================================================
               FARMER ACHIEVEMENTS FROM SERVER
            ===================================================== */

            farmers50AchievementUnlocked =
                Boolean(
                    data.farmers50Achievement
                );

            farmers125AchievementUnlocked =
                Boolean(
                    data.farmers125Achievement
                );

            farmers180AchievementUnlocked =
                Boolean(
                    data.farmers180Achievement
                );

            farmers275AchievementUnlocked =
                Boolean(
                    data.farmers275Achievement
                );

            farmers400AchievementUnlocked =
                Boolean(
                    data.farmers400Achievement
                );

            farmers600AchievementUnlocked =
                Boolean(
                    data.farmers600Achievement
                );

            farmers800AchievementUnlocked =
                Boolean(
                    data.farmers800Achievement
                );

            farmers950AchievementUnlocked =
                Boolean(
                    data.farmers950Achievement
                );

            farmers1100AchievementUnlocked =
                Boolean(
                    data.farmers1100Achievement
                );


            /* =========================
               UPDATE FARMER ACHIEVEMENT UI
            ========================= */
            ...
            /* =====================================================
               FARMER ACHIEVEMENT NOTIFICATIONS
            ===================================================== */
            ...
            if (
                ...
                data.newlyUnlocked.forEach(
                    (achievement) => {

                        showAchievementUnlocked(
                            `${achievement.threshold} Farmers`,
                            `+${achievement.reward} CPS`
                        );
            ...
            /* =====================================================
               CPS ACHIEVEMENTS FROM SERVER
            ===================================================== */

            cps100AchievementUnlocked =
                Boolean(
                    data.cps100Achievement
                );

            cps500AchievementUnlocked =
                Boolean(
                    data.cps500Achievement
                );

            cps800AchievementUnlocked =
                Boolean(
                    data.cps800Achievement
                );

            cps1250AchievementUnlocked =
                Boolean(
                    data.cps1250Achievement
                );

            cps1750AchievementUnlocked =
                Boolean(
                    data.cps1750Achievement
                );

            cps2500AchievementUnlocked =
                Boolean(
                    data.cps2500Achievement
                );

            cps3500AchievementUnlocked =
                Boolean(
                    data.cps3500Achievement
                );

            cps4500AchievementUnlocked =
                Boolean(
                    data.cps4500Achievement
                );

            cps6000AchievementUnlocked =
                Boolean(
                    data.cps6000Achievement
                );


            /* =========================
               UPDATE CPS ACHIEVEMENT UI
            ========================= */
            ...
            /* =====================================================
               CPS ACHIEVEMENT NOTIFICATIONS
            ===================================================== */

            if (
                Array.isArray(
                    data.newlyUnlockedCpsAchievements
                )
            ) {

                data.newlyUnlockedCpsAchievements.forEach(
                    (achievement) => {

                        showAchievementUnlocked(
                            `${achievement.threshold} CPS`,
                            `+${achievement.reward}%`
                        );
            ...
            /* =========================
               COMPLETE FARMER DAILY QUEST
            ========================= */

            await completeFarmerDailyQuest();
...
let farmerDailyQuestCompleted = false;
...
function updateFarmerDailyQuest() {
 ...
 async function completeFarmerDailyQuest() {
     ...
     try {
         ...
         totalGemsEarned =
             Number(data.totalGemsEarned) || 0;

         totalGemsBonus =
             Number(data.totalGemsBonus) || 0;
         ...
         if (Array.isArray(data.newlyUnlocked)) {
             ...
             data.newlyUnlocked.forEach(
                 achievement => {
                     ...
                     showAchievementUnlocked(
                         `${achievement.threshold} GEMS`,
                         `+${achievement.reward}% TOTAL GEMS`
                     );
...
// =========================
// UPDATE FARMER SHOP PRICES
// =========================

function updateFarmerShopPrices() {
    ...
    farmerShopItems.forEach(
        (button) => {
            ...
            if (
                priceElement &&
                shopFarmerCosts[farmerIndex] !== undefined
            ) {

                priceElement.textContent =
                    formatNumber(
                        shopFarmerCosts[farmerIndex]
                    ) + " 🪙";
            }
...
function updateTapPowerPrices() {

    farmerShopItems.forEach((button) => {
...
if (button.dataset.lucky) {
    ...
    console.log("🍀 LUCKY COIN CLICKED");
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/lucky-coin",
                {
                    method: "POST",
        ...
        luckyCoinMultiplier =
            Number(
                data.luckyCoinMultiplier
            );

        luckyCoinMultiplierCost =
            Number(
                data.luckyCoinMultiplierCost
            );
...
if (button.dataset.energy) {
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/energy-upgrade",
                {
                    method: "POST",
        ...
        energyFlatBonus =
            Number(data.energyFlatBonus);

        energyUpgradeCost =
            Number(data.energyUpgradeCost);
...
if (button.dataset.power) {
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/tap-power",
                {
                    method: "POST",
        ...
        tapPowerCosts =
    Array.isArray(data.tapPowerCosts)
        ? data.tapPowerCosts
        : tapPowerCosts;

// =========================
// UPDATE TAP POWER PRICE
// =========================
...
if (amount === 2) {
    await completeTapPower1Quest();
}

if (amount === 3) {
    await completeTapPower2Quest();
}

if (amount === 4) {
    await completeTapPower3Quest();
}

if (amount === 5) {
    await completeTapPower4Quest();
}

if (amount === 6) {
    await completeTapPower5Quest();
}
...
if (button.dataset.farmer) {
    ...
    try {
        ...
        /* =========================
           BUY FARMER FROM SERVER
        ========================= */

        console.log(
            "🌾 SENDING FARMER PURCHASE TO SERVER:",
            {
                farmerIndex: index,
                initDataExists: !!telegramInitData
            }
        );


        const response = await fetch(
            "https://server-72ja.onrender.com/api/buy-farmer",
            {
                method: "POST",
        ...
        console.log(
            "🌾 FARMER PURCHASE SERVER RESPONSE:",
            data
        );
        ...
        if (
            Array.isArray(data.farmerCosts)
        ) {
        ...
        if (
            Array.isArray(data.newlyUnlocked)
        ) {
        ...
        if (
            ...
            for (
                const achievement
                of data.newlyUnlockedCpsAchievements
            ) {
        ...
        if (index === 0) {
            await completeNewbieQuest();
        }

        if (index === 1) {
            await completeBeginnerQuest();
        }

        if (index === 2) {
            await completeIntermediateQuest();
        }

        if (index === 3) {
            await completeExpertQuest();
        }

        if (index === 4) {
            await completeMasterQuest();
        }
...
} // closes if (button.dataset.farmer)
...
let newbieQuestCompleted = false;
...
/* =========================
   COMPLETE NEWBIE QUEST
========================= */

async function completeNewbieQuest() {
...
async function completeBeginnerQuest() {
    ...
    try {
        ...
        console.log(
            "BEGINNER QUEST SERVER RESPONSE:",
            data
        );
...
async function completeIntermediateQuest() {
    ...
    try {
        ...
        console.log(
            "INTERMEDIATE QUEST SERVER RESPONSE:",
            data
        );
        ...
        console.error(
            "Intermediate quest request error:",
            error
        );
    }
}
...
async function completeExpertQuest() {
...
async function completeMasterQuest() {
...
let tapPower1QuestCompleted = false;
...
async function completeTapPower1Quest() {
    ...
    try {
        ...
        console.log(
            "TAP POWER I QUEST SERVER RESPONSE:",
            data
        );
        ...
        console.error(
            "Tap Power I quest request error:",
            error
        );
    }
}
...
let tapPower2QuestCompleted = false;
...
async function completeTapPower2Quest() {
    ...
    try {
        ...
        console.log(
            "TAP POWER II QUEST SERVER RESPONSE:",
            data
        );
        ...
        console.error(
            "Tap Power II quest request error:",
            error
        );
    }
}
...
let tapPower3QuestCompleted = false;
...
async function completeTapPower3Quest() {
    ...
    try {
        ...
        console.log(
            "TAP POWER III QUEST SERVER RESPONSE:",
            data
        );
        ...
        console.error(
            "Tap Power III quest request error:",
            error
        );
    }
}
...
let tapPower4QuestCompleted = false;
...
async function completeTapPower4Quest() {
    ...
    try {
        ...
        console.log(
            "TAP POWER IV QUEST SERVER RESPONSE:",
            data
        );
        ...
        console.error(
            "Tap Power IV quest request error:",
            error
        );
    }
}
...
let tapPower5QuestCompleted = false;
...
async function completeTapPower5Quest() {
    ...
    try {
        ...
        console.log(
            "TAP POWER V QUEST SERVER RESPONSE:",
            data
        );
        ...
        console.error(
            "Tap Power V quest request error:",
            error
        );
    }
}
...
setInterval(
    async function () {
        ...
        try {
            ...
            /* =========================
               REQUEST SERVER INCOME
            ========================= */

            const response =
                await fetch(
                    "https://server-72ja.onrender.com/api/passive-income",
                    {
                        method: "POST",
            ...
            console.log(
                "PASSIVE INCOME SERVER RESPONSE:",
                data
            );
            ...
            await countCoinQuest(
                Number(data.passiveCoins) || 0
            );
...
let clickQuestCount = 0;

let clickQuestCompleted = false;

let coinQuestCount = 0;

let coinQuestCompleted = false;

let tapPowerQuestCompleted =
    localStorage.getItem("tapPowerQuestCompleted") === "true";

let onlineQuestMinutes = 0;

let onlineQuestCompleted = false;
...
setInterval(async function () {
    ...
    if (!onlineQuestCompleted) {
        ...
        if (totalTimePlayed % 60 === 0) {
            ...
            console.log(
                "⏱️ 60 SECONDS REACHED - SENDING ONLINE QUEST"
            );

            await updateOnlineQuestProgress(1);
...
/* =========================================================
   ONLINE QUEST SERVER PROGRESS
========================================================= */

async function updateOnlineQuestProgress(minutes) {
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/online-quest-progress",
                {
                    method: "POST",
        ...
        onlineQuestMinutes =
            Number(
                data.onlineQuestMinutes
            ) || 0;

       

        /* =========================
           QUEST REACHED 60 MINUTES
        ========================= */
...
/* =========================================================
   COMPLETE ONLINE QUEST
========================================================= */

async function completeOnlineQuest() {
    ...
    try {
        ...
        console.log(
            "⏱️ ONLINE 60 MIN QUEST COMPLETED!"
        );

    } catch (error) {

        console.error(
            "Online quest claim error:",
            error
        );
...
let energyUpgradeQuestCompleted = false;
...
async function completeEnergyUpgradeQuest() {
    ...
    try {
        ...
        totalGemsEarned =
            Number(data.totalGemsEarned);

        totalGemsBonus =
            Number(data.totalGemsBonus);
...
let luckyCoinsQuestCompleted = false;
...
async function completeLuckyCoinsQuest() {
...
let tokenTapPowerQuestCompleted = false;
...
async function completeTokenTapPowerQuest() {
    ...
    try {
        ...
        if (
            ...
            console.error(
                "Token Tap Power quest claim failed:",
                data.error
            );
        ...
        console.log(
            "📺 TOKEN TAP POWER QUEST COMPLETED!"
        );

    } catch (error) {

        console.error(
            "Token Tap Power quest claim error:",
            error
        );
...
let tokenCPSQuestCompleted = false;
...
async function completeTokenCPSQuest() {
    ...
    try {
        ...
        console.log(
            "🪙 TOKEN CPS QUEST COMPLETED!"
        );

    } catch (error) {

        console.error(
            "Token CPS quest claim error:",
            error
        );
...
let fallingTokenQuestCount = 0;

let fallingTokenQuestCompleted = false;
...
async function collectFallingTokenForQuest() {

    console.log(
        "🪙 FALLING QUEST START:",
        {
            count: fallingTokenQuestCount,
            completed: fallingTokenQuestCompleted
        }
    );
    ...
    if (fallingTokenQuestCompleted) {
        ...
        console.log(
            "🪙 FALLING QUEST ALREADY COMPLETED"
        );
    ...
    try {
        ...
        console.log(
            "🪙 SENDING FALLING QUEST REQUEST"
        );

        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/falling-token-quest-progress",
                {
                    method: "POST",
                    headers: {
                        "Content-Type":
        ...
        fallingTokenQuestCount =
            Number(
                data.fallingTokenQuestCount
            ) || 0;

        console.log(
            "🪙 NEW FALLING QUEST COUNT:",
            fallingTokenQuestCount
        );
        ...
        if (
            ...
            console.log(
                "🪙 FALLING QUEST REACHED 5"
            );

            await completeFallingTokenQuest();
        }
...
/* =========================================================
   COMPLETE FALLING TOKEN DAILY QUEST
========================================================= */

async function completeFallingTokenQuest() {
    ...
    try {
        ...
        if (
            ...
            console.error(
                "Falling Tokens quest claim failed:",
                data.error
            );
            return;
        }
        ...
        console.error(
            "Falling Tokens quest claim error:",
            error
        );
    }
}
...
let riceHarvestQuestCount = 0;

let riceHarvestQuestCompleted = false;
...
async function collectRiceForQuest(amount = 1) {
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/rice-harvest-quest-progress",
                {
                    method: "POST",
                    headers: {
                        "Content-Type":
        ...
        if (
            ...
            console.error(
                "Rice harvest quest progress failed:",
                data.error
            );
            return;
        }
        ...
        if (
            ...
            await completeRiceHarvestQuest();
        }
...
/* =========================================================
   COMPLETE RICE HARVEST DAILY QUEST
========================================================= */

async function completeRiceHarvestQuest() {
    ...
    try {
        ...
        if (
            ...
            console.error(
                "Rice harvest quest claim failed:",
                data.error
            );
            return;
        }
        ...
        console.error(
            "Rice harvest quest claim error:",
            error
        );
    }
}
...
let potatoHarvestQuestCount = 0;

let potatoHarvestQuestCompleted = false;
...
async function collectPotatoForQuest(amount = 1) {
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/potato-harvest-quest-progress",
                {
                    method: "POST",
                    headers: {
                        "Content-Type":
        ...
        if (
            ...
            console.error(
                "Potato harvest quest progress failed:",
                data.error
            );
            return;
        }
        ...
        if (
            ...
            await completePotatoHarvestQuest();
        }
...
/* =========================================================
   COMPLETE POTATO HARVEST DAILY QUEST
========================================================= */

async function completePotatoHarvestQuest() {
    ...
    try {
        ...
        if (
            ...
            console.error(
                "Potato harvest quest claim failed:",
                data.error
            );
            return;
        }
        ...
        console.error(
            "Potato harvest quest claim error:",
            error
        );
    }
}
...
/* =========================================================
   HARVEST 50 CORN DAILY QUEST
========================================================= */

let cornHarvestQuestCount = 0;

let cornHarvestQuestCompleted = false;
...
/* =========================================================
   HARVEST CORN DAILY QUEST
========================================================= */

async function collectCornForQuest(amount = 1) {
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/corn-harvest-quest-progress",
                {
                    method: "POST",
        ...
        if (
            ...
            console.error(
                "Corn harvest quest progress failed:",
                data.error
            );
        ...
        if (
            ...
            await completeCornHarvestQuest();
        }
...
async function completeCornHarvestQuest() {
    ...
    try {
        ...
        console.log(
            "🌽 CORN HARVEST QUEST COMPLETED!"
        );

    } catch (error) {

        console.error(
            "Corn harvest quest completion error:",
            error
        );
...
let moonPowerQuestCompleted = false;
...
/* =========================================================
   COMPLETE MOON POWER DAILY QUEST
========================================================= */

async function completeMoonPowerQuest() {
...
/* =========================================================
   COMPLETE NIGHT INCOME DAILY QUEST
========================================================= */

async function completeNightIncomeQuest() {
    ...
    try {
        ...
        console.log(
            "NIGHT INCOME QUEST CLAIM RESPONSE:",
            data
        );
        ...
        if (
            ...
            console.error(
                "Night Income quest claim failed:",
                data.error
            );
        ...
        console.error(
            "Night Income quest claim error:",
            error
        );
    }
}
...
/* =========================================================
   COLLECT NIGHT INCOME QUEST PROGRESS
========================================================= */

async function collectNightIncomeForQuest(amount = 1) {
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/night-income-quest-progress",
                {
                    method: "POST",
        ...
        console.log(
            "NIGHT INCOME QUEST PROGRESS RESPONSE:",
            data
        );
        ...
        if (
            ...
            console.error(
                "Night Income quest progress failed:",
                data.error
            );
        ...
        if (
            nightIncomeQuestCount >= 1 &&
            !nightIncomeQuestCompleted
        ) {

            await completeNightIncomeQuest();
        }
...
/* =========================================================
   COMPLETE RICE SOUP DAILY QUEST
========================================================= */

async function completeRiceSoupQuest() {
    ...
    try {
        ...
        console.log(
            "RICE SOUP QUEST CLAIM RESPONSE:",
            data
        );
...
/* =========================================================
   COLLECT RICE SOUP QUEST PROGRESS
========================================================= */

async function collectRiceSoupForQuest(amount = 1) {
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/rice-soup-quest-progress",
                {
                    method: "POST",
        ...
        console.log(
            "RICE SOUP QUEST PROGRESS RESPONSE:",
            data
        );
        ...
        if (
            ...
            console.error(
                "Rice Soup quest progress failed:",
                data.error
            );
        ...
        if (
            riceSoupQuestCount >= 1 &&
            !riceSoupQuestCompleted
        ) {

            await completeRiceSoupQuest();
        }
...
let potatoSoupQuestCount = 0;

let potatoSoupQuestCompleted = false;
...
/* =========================================================
   COMPLETE POTATO SOUP DAILY QUEST
========================================================= */

async function completePotatoSoupQuest() {
    ...
    try {
        ...
        console.log(
            "POTATO SOUP QUEST CLAIM RESPONSE:",
            data
        );
        ...
        if (
            ...
            console.error(
                "Potato Soup quest claim failed:",
                data.error
            );
...
/* =========================================================
   COLLECT POTATO SOUP QUEST PROGRESS
========================================================= */

async function collectPotatoSoupForQuest(
    amount = 1
) {
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/potato-soup-quest-progress",
                {
                    method: "POST",
        ...
        console.log(
            "POTATO SOUP QUEST PROGRESS RESPONSE:",
            data
        );
        ...
        if (
            ...
            console.error(
                "Potato Soup quest progress failed:",
                data.error
            );
        ...
        if (
            potatoSoupQuestCount >= 1 &&
            !potatoSoupQuestCompleted
        ) {

            await completePotatoSoupQuest();
        }
...
let cornSoupQuestCount = 0;

let cornSoupQuestCompleted = false;
...
/* =========================================================
   COMPLETE CORN SOUP DAILY QUEST
========================================================= */

async function completeCornSoupQuest() {
    ...
    try {
        ...
        console.log(
            "CORN SOUP QUEST CLAIM RESPONSE:",
            data
        );
...
/* =========================================================
   COLLECT CORN SOUP QUEST PROGRESS
========================================================= */

async function collectCornSoupForQuest(
    amount = 1
) {
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/corn-soup-quest-progress",
                {
                    method: "POST",
        ...
        console.log(
            "CORN SOUP QUEST PROGRESS RESPONSE:",
            data
        );
        ...
        if (
            ...
            console.error(
                "Corn Soup quest progress failed:",
                data.error
            );
        ...
        if (
            cornSoupQuestCount >= 1 &&
            !cornSoupQuestCompleted
        ) {

            await completeCornSoupQuest();
        }
...
function updateLuckyCoinMultiplierPrice() {

    const priceElement =
        document.getElementById(
            "luckyCoinMultiplierPrice"
        );
...
/* =========================================
   DAILY QUEST RESET COMPATIBILITY
...
async function resetAllDailyQuests() {
    ...
    console.log(
        "DAILY QUEST RESET CHECK - SERVER AUTHORITATIVE"
    );

    /*
       Daily quest progress is controlled by
       the server and daily_quest_claims.

       DO NOT reset quest counters here.
       DO NOT reset energyRefillsUsed here.
       DO NOT use localStorage for daily resets.
       DO NOT call old /api/reset-* endpoints.
    */

    if (typeof updateEnergyRefillButton === "function") {
        updateEnergyRefillButton();
    }

    console.log(
        "DAILY QUEST RESET HANDLED BY SERVER"
    );
}
...
console.log(
    "🟢 NEWBIE QUEST LOAD DEBUG:",
    data.newbieDailyQuestCompleted,
    data
);
...
console.log(
    "🖱️ SERVER CLICK QUEST COUNT:",
    data.clickQuestCount
);

console.log(
    "🖱️ SERVER CLICK QUEST COMPLETED:",
    data.clickQuestCompleted
);

console.log(
    "API NIGHT TAP COST:",
    data.nightTapUpgradeCost
);

console.log(
    "API NIGHT TAP LEVEL:",
    data.nightTapUpgradeLevel
);
...
luckyCoinMultiplier =
    Number(data.luckyCoinMultiplier) || 0;

luckyCoinMultiplierCost =
    Number(data.luckyCoinMultiplierCost) || 5000;

luckyCoinChance =
    Number(data.user?.lucky_coin_chance) || 1;

tokenCPSQuestCompleted =
    data.tokenCPSQuestCompleted === true;
...
nightIncomeQuestCompleted =
    data.nightIncomeQuestCompleted === true;

updateNightIncomeQuest();

console.log(
    "🌙 NIGHT INCOME QUEST LOADED:",
    nightIncomeQuestCount,
    nightIncomeQuestCompleted
);
...
cornSoupQuestCompleted =
    data.cornSoupQuestCompleted === true;

potatoSoupQuestCompleted =
    data.potatoSoupQuestCompleted === true;

riceSoupQuestCompleted =
    data.riceSoupQuestCompleted === true;
...
console.log(
    "🍚 RICE SOUP QUEST LOADED:",
    riceSoupQuestCount,
    riceSoupQuestCompleted
);

console.log(
    "🥔 POTATO SOUP QUEST LOADED:",
    potatoSoupQuestCount,
    potatoSoupQuestCompleted
);

console.log(
    "🌽 CORN SOUP QUEST LOADED:",
    cornSoupQuestCount,
    cornSoupQuestCompleted
);
...
clickQuestCompleted =
    data.clickQuestCompleted === true;

updateClick500Quest();

newbieQuestCompleted =
    data.newbieDailyQuestCompleted === true;

console.log(
    "🟢 NEWBIE QUEST VARIABLE:",
    newbieQuestCompleted
);
...
totalCoinsEarned =
    Number(data.user?.total_coins_earned) || 0;
...
coinQuestCount =
    Number(data.coinQuestCount) || 0;

coinQuestCompleted =
    data.coinQuestCompleted === true;
...
onlineQuestCompleted =
    data.onlineQuestCompleted === true;

console.log(
    "⏱️ ONLINE QUEST LOADED FROM SERVER:",
    onlineQuestMinutes
);
...
tokenTapPowerQuestCompleted =
    data.tokenTapPowerQuestCompleted === true;
...
fallingTokenQuestCompleted =
    data.fallingTokensQuestCompleted === true;
...
riceHarvestQuestCompleted =
    data.riceHarvestQuestCompleted === true;
...
potatoHarvestQuestCompleted =
    data.potatoHarvestQuestCompleted === true;
...
cornHarvestQuestCompleted =
    data.cornHarvestQuestCompleted === true;

/* =========================
   UPDATE HARVEST QUEST UI
========================= */
...
console.log(
    "🌾 RICE HARVEST QUEST LOADED:",
    riceHarvestQuestCount,
    riceHarvestQuestCompleted
);

console.log(
    "🥔 POTATO HARVEST QUEST LOADED:",
    potatoHarvestQuestCount,
    potatoHarvestQuestCompleted
);

console.log(
    "🌽 CORN HARVEST QUEST LOADED:",
    cornHarvestQuestCount,
    cornHarvestQuestCompleted
);
...
moonPowerQuestCount =
    Number(
        data.moonPowerQuestCount
    ) || 0;

moonPowerQuestCompleted =
    data.moonPowerQuestCompleted === true;

updateMoonPowerQuest();

console.log(
    "🌙 MOON POWER QUEST LOADED:",
    moonPowerQuestCount,
    moonPowerQuestCompleted
);

farmerDailyQuestCompleted =
    data.farmerDailyQuestCompleted === true;

updateFarmerDailyQuest();

/* =========================
   LOAD TAP POWER QUESTS
========================= */

tapPower1QuestCompleted =
    data.tapPower1QuestCompleted === true;

tapPower2QuestCompleted =
    data.tapPower2QuestCompleted === true;

tapPower3QuestCompleted =
    data.tapPower3QuestCompleted === true;

tapPower4QuestCompleted =
    data.tapPower4QuestCompleted === true;

tapPower5QuestCompleted =
    data.tapPower5QuestCompleted === true;

/* =========================
   UPDATE TAP POWER QUESTS
========================= */
...
/* =========================
   LOAD ENERGY UPGRADE QUEST
========================= */

energyUpgradeQuestCompleted =
    data.energyUpgradeQuestCompleted === true;

updateEnergyUpgradeQuest();

/* =========================
   LOAD LUCKY COIN QUEST
========================= */

luckyCoinsQuestCompleted =
    data.luckyCoinsQuestCompleted === true;

console.log(
    "🪙 LUCKY COIN QUEST FROM SERVER:",
    data.luckyCoinsQuestCompleted
);

updateLuckyCoinsQuest();

/* =========================
   LOAD TOKEN CPS QUEST
========================= */
...
console.log(
    "🪙 TOKEN CPS QUEST FROM SERVER:",
    data.tokenCPSQuestCompleted
);

updateTokenCPSQuest();

/* =========================
   LOAD TOKEN TAP POWER QUEST
========================= */
...
console.log(
    "🪙 TOKEN TAP POWER QUEST FROM SERVER:",
    data.tokenTapPowerQuestCompleted
);

updateTokenTapPowerQuest();

/* =========================
   LOAD GEMS FROM SERVER
========================= */
...
energyBonusPercent =
    Number(
        data.user?.energy_bonus_percent
    ) || 0;

/* =========================
   LOAD ENERGY UPGRADE COST
========================= */

energyUpgradeCost =
    Number(data.energyUpgradeCost) || 5000;

console.log(
    "⚡ SERVER ENERGY UPGRADE COST:",
    energyUpgradeCost
);
...
caseSpinsUsed =
    Number(
        data.dailyCaseSpinsUsed
    ) || 0;

console.log(
    "🎁 SERVER DAILY CASE SPINS USED:",
    caseSpinsUsed
);

if (
    typeof window.updateDailyCaseSpins ===
    "function"
) {
    window.updateDailyCaseSpins();
}
...
gems100AchievementUnlocked =
    Boolean(data.gemsAchievement100);

gems250AchievementUnlocked =
    Boolean(data.gemsAchievement250);

gems500AchievementUnlocked =
    Boolean(data.gemsAchievement500);

gems1000AchievementUnlocked =
    Boolean(data.gemsAchievement1000);

gems1750AchievementUnlocked =
    Boolean(data.gemsAchievement1750);

gems3000AchievementUnlocked =
    Boolean(data.gemsAchievement3000);

gems5000AchievementUnlocked =
    Boolean(data.gemsAchievement5000);

gems7500AchievementUnlocked =
    Boolean(data.gemsAchievement7500);

gems11000AchievementUnlocked =
    Boolean(data.gemsAchievement11000);

console.log(
    "SERVER GEM ACHIEVEMENTS:",
    gems100AchievementUnlocked,
    gems250AchievementUnlocked,
    gems500AchievementUnlocked,
    gems1000AchievementUnlocked,
...
/* =========================
   LOAD CLICK ACHIEVEMENTS
========================= */

clicks1000AchievementUnlocked =
    Boolean(data.clicks1000Achievement);

clicks5000AchievementUnlocked =
    Boolean(data.clicks5000Achievement);

clicks12500AchievementUnlocked =
    Boolean(data.clicks12500Achievement);

clicks25000AchievementUnlocked =
    Boolean(data.clicks25000Achievement);

clicks50000AchievementUnlocked =
    Boolean(data.clicks50000Achievement);

clicks80000AchievementUnlocked =
    Boolean(data.clicks80000Achievement);

clicks125000AchievementUnlocked =
    Boolean(data.clicks125000Achievement);

clicks180000AchievementUnlocked =
    Boolean(data.clicks180000Achievement);

clicks250000AchievementUnlocked =
    Boolean(data.clicks250000Achievement);

console.log(
    "🖱️ SERVER CLICK ACHIEVEMENTS:",
    clicks1000AchievementUnlocked,
    clicks5000AchievementUnlocked,
    clicks12500AchievementUnlocked,
    clicks25000AchievementUnlocked,
...
/* =========================
   LOAD CPS ACHIEVEMENTS
========================= */

cps100AchievementUnlocked =
    Boolean(data.cps100Achievement);

cps500AchievementUnlocked =
    Boolean(data.cps500Achievement);

cps800AchievementUnlocked =
    Boolean(data.cps800Achievement);

cps1250AchievementUnlocked =
    Boolean(data.cps1250Achievement);

cps1750AchievementUnlocked =
    Boolean(data.cps1750Achievement);

cps2500AchievementUnlocked =
    Boolean(data.cps2500Achievement);

cps3500AchievementUnlocked =
    Boolean(data.cps3500Achievement);

cps4500AchievementUnlocked =
    Boolean(data.cps4500Achievement);

cps6000AchievementUnlocked =
    Boolean(data.cps6000Achievement);

console.log(
    "⚡ SERVER CPS ACHIEVEMENTS:",
    cps100AchievementUnlocked,
    cps500AchievementUnlocked,
    cps800AchievementUnlocked,
    cps1250AchievementUnlocked,
...
/* =========================
   LOAD FARMER ACHIEVEMENTS
========================= */

farmers50AchievementUnlocked =
    Boolean(data.farmers50Achievement);

farmers125AchievementUnlocked =
    Boolean(data.farmers125Achievement);

farmers180AchievementUnlocked =
    Boolean(data.farmers180Achievement);

farmers275AchievementUnlocked =
    Boolean(data.farmers275Achievement);

farmers400AchievementUnlocked =
    Boolean(data.farmers400Achievement);

farmers600AchievementUnlocked =
    Boolean(data.farmers600Achievement);

farmers800AchievementUnlocked =
    Boolean(data.farmers800Achievement);

farmers950AchievementUnlocked =
    Boolean(data.farmers950Achievement);

farmers1100AchievementUnlocked =
    Boolean(data.farmers1100Achievement);
...
console.log(
    "SERVER TOTAL GEMS EARNED:",
    totalGemsEarned
);

console.log(
    "SERVER TOTAL GEMS BONUS:",
    totalGemsBonus
);

/* =========================
   LOAD NIGHT LILIES FROM SERVER
========================= */

nightLilies =
    Number(data.nightLilies) || 0;
...
/* =========================
   LOAD MOON POWER VALUES
========================= */

nightTapUpgradeCost =
    Number(data.nightTapUpgradeCost) || 10;

nightTapUpgradeLevel =
    Number(data.nightTapUpgradeLevel) || 0;

console.log(
    "SERVER MOON POWER COST:",
    nightTapUpgradeCost
);

console.log(
    "SERVER MOON POWER LEVEL:",
    nightTapUpgradeLevel
);

console.log(
    "FRONTEND COST VARIABLE:",
    nightTapUpgradeCost
);
...
console.log(
    "FRONTEND HTML COST:",
    document.getElementById("nightTapUpgradeCost")?.textContent
);
...
nightBonus =
    Number(data.nightBonus) || 0;

nightCpsUpgradeLevel =
    Number(data.nightCpsUpgradeLevel) || 0;

nightCpsUpgradeCost =
    Number(data.nightCpsUpgradeCost) || 10;
...
console.log(
    "SERVER NIGHT INCOME LEVEL:",
    nightCpsUpgradeLevel
);

console.log(
    "SERVER NIGHT INCOME COST:",
    nightCpsUpgradeCost
);

/* =========================
   UPDATE NIGHT INCOME PRICE
========================= */

const nightCpsCostElement =
    document.getElementById(
        "nightCpsUpgradeCost"
    );

if (nightCpsCostElement) {

    nightCpsCostElement.textContent =
        formatNumber(
            nightCpsUpgradeCost
        );
...
upgradeCost =
    Number(data.user?.upgrade_cost) || 50;
...
console.log(
    "NEW SERVER TAP POWER COSTS:",
    tapPowerCosts
);

coinsPerSecondMultiplier =
    Number(
        data.user?.coins_per_second_multiplier
    ) || 1;
...
console.log(
    "TOKEN DISPLAY AFTER SERVER LOAD:",
    document.getElementById("tokens")?.textContent
);

/* =========================
   LOAD RICE FROM SERVER
========================= */

rice =
    Number(data.user?.rice) || 0;
...
/* =========================
   LOAD SEEDS FROM SERVER
========================= */

riceSeeds =
    Number(data.user?.rice_seeds) || 0;

potatoSeeds =
    Number(data.user?.potato_seeds) || 0;

cornSeeds =
    Number(data.user?.corn_seeds) || 0;
...
console.log(
    "SERVER RICE GROW UPGRADES:",
    data.riceGrowUpgrades
);

console.log(
    "SERVER POTATO GROW UPGRADES:",
    data.potatoGrowUpgrades
);

console.log(
    "SERVER CORN GROW UPGRADES:",
    data.cornGrowUpgrades
);
...
/* =========================
   LOAD UNLOCKED FIELD PLOTS
========================= */

unlockedFieldPlots =
    data.user?.unlocked_field_plots || [];

console.log(
    "SERVER UNLOCKED FIELD PLOTS:",
    unlockedFieldPlots
);
...
farmers =
    Number(data.user?.farmers) || 0;

perSecond =
    Number(data.user?.per_second) || 0;
...
let loadedFarmerCosts =
    data.user?.farmer_costs;
...
if (typeof loadedFarmerCosts === "string") {
    ...
    try {
        ...
        console.error(
            "Failed to parse farmer_costs:",
            error
        );

        loadedFarmerCosts = null;
    }
}

/* =========================
   APPLY SERVER FARMER COSTS
========================= */

if (Array.isArray(loadedFarmerCosts)) {

    shopFarmerCosts =
        loadedFarmerCosts;

    console.log(
        "🌾 SERVER FARMER COSTS APPLIED:",
        shopFarmerCosts
    );
...
console.log(
    "SERVER MAIN FARMER COST:",
    farmerCost
);

/* =========================
   LOAD RICE SOUP LEVEL
========================= */

riceSoupLevel =
    Number(data.user?.rice_soup_level) || 0;

console.log(
    "SERVER RICE SOUP LEVEL:",
    riceSoupLevel
);
...
tonBonus =
    Number(data.tonBonus || 0);

cpsBonus =
    Number(data.cpsBonus || 0);

tapBonus =
    Number(data.tapBonus || 0);

power =
    Number(data.tapPower || power);
...
updateLuckyCoinMultiplierPrice();

/* =========================
   SERVER STATS ARE LOADED
========================= */
...
console.log("10K COINS QUEST FROM SERVER:", {
    count: data.coinQuestCount,
    completed: data.coinQuestCompleted
});
...
mainQuestBtn.addEventListener(
    "click",
    function () {

        console.log(
            "MAIN QUEST BUTTON CLICKED"
        );
...
/* =========================
   COMPLETE CLICK 500 QUEST
========================= */

async function completeClick500Quest() {
...
cornBtn.addEventListener(
    ...
    async function () {
        ...
        try {
            ...
            const response =
                await fetch(
                    "https://server-72ja.onrender.com/api/click-quest-progress",
                    {
                        method: "POST",
                        headers: {
                            "Content-Type":
...
/* =========================
   COMPLETE 10,000 COIN QUEST
========================= */

async function completeCoin10000Quest() {
...
async function countCoinQuest(coins) {
    ...
    try {
        ...
        const response =
            await fetch(
                "https://server-72ja.onrender.com/api/coin-quest-progress",
                {
                    method: "POST",
                    headers: {
                        "Content-Type":
    ...
    buyBoostedCoins.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/buy-boosted-coins",
                        {
                            method: "POST",
    ...
    buyCoinsPerSecond.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/buy-cps-multiplier",
                        {
                            method: "POST",
                ...
                console.log(
                    "CPS MULTIPLIER SERVER RESPONSE:",
                    data
                );
                ...
                if (
                    ...
                    console.error(
                        "CPS Multiplier purchase failed:",
                        data.error
                    );
...
if (buyEnergyPercent) {
    ...
    buyEnergyPercent.addEventListener(
        ...
        async function () {
            ...
            buyEnergyPercent.disabled = true;
            ...
            try {
                ...
                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/buy-energy-percent",
                        {
                            method: "POST",
                ...
                console.log(
                    "ENERGY PERCENT SERVER RESPONSE:",
                    data
                );
                ...
                if (
                    ...
                    console.error(
                        "Energy Percent purchase failed:",
                        data.error
                    );
    ...
    buyLuckyChance.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/buy-lucky-chance",
                        {
                            method: "POST",
...
async function saveLeaderboardStats() {
    ...
    /* =====================================================
       DO NOT SAVE BEFORE SERVER DATA IS LOADED
    ===================================================== */

    if (!serverStatsLoaded) {

        console.log(
            "⏳ Server stats not loaded yet. Skipping leaderboard save."
        );
    ...
    if (!telegramInitData) {
        ...
        console.log(
            "⏳ Telegram initData missing. Skipping leaderboard save."
        );
...
/* =========================================================
   BUY +5% TOTAL BONUS WITH TON
========================================================= */

async function buyTotalBonusWithTON() {
    ...
    try {
        ...
        if (
            ...
            alert(
                `✅ Payment successful!\n\n🪙 +${verifyData.totalBonusAdded}% Total Bonus`
            );
...
/* =========================================================
   BUY +100 CPS +5% TOTAL BONUS WITH TON
========================================================= */

async function buyCpsTotalBonusWithTON() {
    ...
    try {
        ...
        if (
         ...
         alert(
    `✅ Payment successful!\n\n` +
    `🪙 +${verifyData.cpsBonusAdded} CPS\n` +
    `📈 +${verifyData.totalBonusAdded}% Total Bonus\n\n` +
    `CPS Bonus total: ${cpsBonus}\n` +
    `TON Bonus total: ${tonBonus}%\n` +
    `All bonuses: ${getTotalBonus()}%`
);
...
/* =========================================================
   BUY +100 TAP POWER +5% TAP BONUS WITH TON
========================================================= */

async function buyTapPower100WithTON() {
    ...
    if (!wallet) {
        ...
        alert(
            "❌ Please connect your TON testnet wallet first."
        );
    ...
    if (
        ...
        alert(
            "❌ Please connect a TON testnet wallet."
        );
    ...
    try {
        ...
        console.log(
            "TON TAP POWER +100 VERIFICATION:",
            verifyData
        );
        ...
        if (
            ...
            power =
                Number(
                    verifyData.tapPower || power
                );

            tapBonus =
                Number(
                    verifyData.tapBonus || tapBonus
                );
            ...
            alert(
                `✅ Payment successful!\n\n` +
                `⚡ +${verifyData.tapPowerAdded} Tap Power\n` +
                `📈 +${verifyData.tapBonusAdded}% Tap Power Bonus\n\n` +
                `Tap Power total: ${power}\n` +
                `Tap Bonus total: ${tapBonus}%`
            );
 ...
 /* =========================================================
    TON +100 TAP POWER +5% BONUS BUTTON
 ========================================================= */

const buyTapPower100TONButton =
    document.getElementById(
        "buyTapPower100TON"
    );

if (buyTapPower100TONButton) {

    buyTapPower100TONButton.onclick =
        async function () {

            if (
                buyTapPower100TONButton.disabled
            ) {
                return;
            }
...
/* =========================================================
   TON CPS + TOTAL BONUS BUTTON
========================================================= */

const buyCpsTotalBonusTONButton =
    document.getElementById(
        "buyCpsTotalBonusTON"
    );

if (buyCpsTotalBonusTONButton) {

    buyCpsTotalBonusTONButton.onclick =
        async function () {

            if (
                buyCpsTotalBonusTONButton.disabled
            ) {
                return;
            }
...
/* =========================================================
   TON TOTAL BONUS BUTTON
========================================================= */

const buyTotalBonusTONButton =
    document.getElementById("buyTotalBonusTON");

if (buyTotalBonusTONButton) {

    buyTotalBonusTONButton.onclick =
        async function () {

            if (
                buyTotalBonusTONButton.disabled
            ) {
                return;
            }
...
async function giveGems(amount) {
    ...
    try {
        ...
        const response = await fetch(
            "https://server-72ja.onrender.com/api/give-gems",
            {
                method: "POST",
        ...
        if (data.achievements) {
            ...
            gems100AchievementUnlocked =
                Boolean(data.achievements.gems100);

            gems250AchievementUnlocked =
                Boolean(data.achievements.gems250);

            gems500AchievementUnlocked =
                Boolean(data.achievements.gems500);

            gems1000AchievementUnlocked =
                Boolean(data.achievements.gems1000);

            gems1750AchievementUnlocked =
                Boolean(data.achievements.gems1750);

            gems3000AchievementUnlocked =
                Boolean(data.achievements.gems3000);

            gems5000AchievementUnlocked =
                Boolean(data.achievements.gems5000);

            gems7500AchievementUnlocked =
                Boolean(data.achievements.gems7500);

            gems11000AchievementUnlocked =
                Boolean(data.achievements.gems11000);
        ...
        async function () {
            ...
            if (!telegramInitData) {
                ...
                caseResult.textContent =
                    "Telegram data is not available.";
            ...
            try {
                ...
                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/daily-case",
                        {
                            method: "POST",
            ...
            /* =============================================
               PUT SERVER WINNER INTO CENTER CARD
            ============================================= */
            ...
            setTimeout(
                function () {
                    ...
                    totalCoinsEarned =
                        Number(
                            serverData.totalCoinsEarned
                        ) || 0;


                    totalGemsEarned =
                        Number(
                            serverData.totalGemsEarned
                        ) || 0;
                    ...
                    /* =====================================
                       UPDATE COIN ACHIEVEMENT FLAGS
                    ===================================== */

                    coins1MAchievementUnlocked =
                        Boolean(
                            serverData.coinsAchievement1m
                        );

                    coins10MAchievementUnlocked =
                        Boolean(
                            serverData.coinsAchievement10m
                        );

                    coins25MAchievementUnlocked =
                        Boolean(
                            serverData.coinsAchievement25m
                        );

                    coins75MAchievementUnlocked =
                        Boolean(
                            serverData.coinsAchievement75m
                        );

                    coins225MAchievementUnlocked =
                        Boolean(
                            serverData.coinsAchievement225m
                        );

                    coins750MAchievementUnlocked =
                        Boolean(
                            serverData.coinsAchievement750m
                        );

                    coins1BAchievementUnlocked =
                        Boolean(
                            serverData.coinsAchievement1b
                        );

                    coins5BAchievementUnlocked =
                        Boolean(
                            serverData.coinsAchievement5b
                        );

                    coins10BAchievementUnlocked =
                        Boolean(
                            serverData.coinsAchievement10b
                        );


                    /* =====================================
                       UPDATE COIN ACHIEVEMENT BONUS
                    ===================================== */

                    coinAchievementBonus =
                        Number(
                            serverData.totalCoinBonus
                        ) || 0;
                    ...
                    /* =====================================
                       SHOW NEW COIN ACHIEVEMENTS
                    ===================================== */

                    if (
                        Array.isArray(
                            serverData.newlyUnlockedCoinAchievements
                        )
                    ) {

                        for (
                            const achievement
                            of serverData.newlyUnlockedCoinAchievements
                        ) {
                    ...
                    /* =====================================
                       UPDATE GEM ACHIEVEMENT FLAGS
                    ===================================== */

                    gems100AchievementUnlocked =
                        Boolean(
                            serverData.gemsAchievement100
                        );

                    gems250AchievementUnlocked =
                        Boolean(
                            serverData.gemsAchievement250
                        );

                    gems500AchievementUnlocked =
                        Boolean(
                            serverData.gemsAchievement500
                        );

                    gems1000AchievementUnlocked =
                        Boolean(
                            serverData.gemsAchievement1000
                        );

                    gems1750AchievementUnlocked =
                        Boolean(
                            serverData.gemsAchievement1750
                        );

                    gems3000AchievementUnlocked =
                        Boolean(
                            serverData.gemsAchievement3000
                        );

                    gems5000AchievementUnlocked =
                        Boolean(
                            serverData.gemsAchievement5000
                        );

                    gems7500AchievementUnlocked =
                        Boolean(
                            serverData.gemsAchievement7500
                        );

                    gems11000AchievementUnlocked =
                        Boolean(
                            serverData.gemsAchievement11000
                        );


                    /* =====================================
                       UPDATE GEM ACHIEVEMENT BONUS
                    ===================================== */

                    gemAchievementBonus =
                        Number(
                            serverData.totalGemBonus
                        ) || 0;
                    ...
                    /* =====================================
                       SHOW NEW GEM ACHIEVEMENTS
                    ===================================== */

                    if (
                        Array.isArray(
                            serverData.newlyUnlocked
                        )
                    ) {

                        for (
                            const achievement
                            of serverData.newlyUnlocked
                        ) {
...
function checkMilestoneReward() {
    // Milestone rewards are now handled securely by the server.
    // Do not modify Tap Power or CPS here.
    return;
}
...
try {
    ...
    const response = await fetch(
        "https://server-72ja.onrender.com/api/falling-token",
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
    ...
    console.log(
        "FALLING TOKEN SERVER RESPONSE:",
        data
    );

    if (!response.ok || !data.success) {
        console.error(
            "Falling token failed:",
            data.error
        );
        return;
    }
...
await collectFallingTokenForQuest();
...
buyTokenTapPower.addEventListener(
    ...
    async function () {
        ...
        buyTokenTapPower.disabled = true;

        try {

            const response = await fetch(
                "https://server-72ja.onrender.com/api/token-tap-power",
                {
                    method: "POST",
            ...
            console.log(
                "TOKEN TAP POWER SERVER RESPONSE:",
                data
            );
            ...
            await completeTokenTapPowerQuest();

        } catch (error) {

            console.error(
                "Token Tap Power request error:",
                error
            );

        } finally {

            buyTokenTapPower.disabled = false;
...
buyTokenCPS.addEventListener(
    ...
    async function () {
        ...
        try {
            ...
            const response =
                await fetch(
                    "https://server-72ja.onrender.com/api/token-cps",
                    {
                        method: "POST",
    ...
    buyRiceSeed.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                const response = await fetch(
                    "https://server-72ja.onrender.com/api/buy-rice-seed",
                    {
                        method: "POST",
    ...
    buyPotatoSeed.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                const response = await fetch(
                    "https://server-72ja.onrender.com/api/buy-potato-seed",
                    {
                        method: "POST",
                ...
                console.log(
                    "BUY POTATO SEED SERVER RESPONSE:",
                    data
                );
    ...
    buyCornSeed.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                const response = await fetch(
                    "https://server-72ja.onrender.com/api/buy-corn-seed",
                    {
                        method: "POST",
...
if (
    ...
    try {
        ...
        const response = await fetch(
            "https://server-72ja.onrender.com/api/unlock-field-plot",
            {
                method: "POST",
        ...
        console.log(
            "UNLOCK FIELD PLOT SERVER RESPONSE:",
            data
        );
        ...
        console.error(
            "Field plot unlock request error:",
            error
        );
...
async function plantCrop(plotNumber, cropType) {
    ...
    try {
        ...
        const response = await fetch(
            "https://server-72ja.onrender.com/api/plant-crop",
            {
                method: "POST",
...
function updateFieldPlots() {
   ...
   /* =====================================================
       SETUP HARVEST TOOLTIPS
    ===================================================== */

    setupFieldHarvestTooltips();
...
async function harvestCrop(plotNumber) {
...
if (cropType === "rice") {
    ...
    try {
        ...
        const response = await fetch(
            "https://server-72ja.onrender.com/api/rice-harvest",
            {
                method: "POST",
        ...
        await collectRiceForQuest(1);


    } catch (error) {

        console.error(
            "Rice harvest request error:",
            error
        );
...
if (cropType === "potato") {
   ...
   console.log(
            "🥔 SENDING POTATO HARVEST REQUEST"
        );

        const response = await fetch(
            "https://server-72ja.onrender.com/api/potato-harvest",
            {
                method: "POST",
        ...
        console.log(
            "POTATO HARVEST SERVER RESPONSE:",
            data
        );
    ...
    potato
       ...
       await collectPotatoForQuest(1);


    } catch (error) {

        console.error(
            "Potato harvest request error:",
            error
        );
...
if (cropType === "corn") {
    ...
    try {
        ...
        const response = await fetch(
            "https://server-72ja.onrender.com/api/corn-harvest",
            {
                method: "POST",
        ...
        await collectCornForQuest(1);


    } catch (error) {

        console.error(
            "Corn harvest request error:",
            error
        );
    ...
    riceGrowUpgrade.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                const response = await fetch(
                    "https://server-72ja.onrender.com/api/rice-grow-upgrade",
                    {
                        method: "POST",
                ...
                console.log(
                    "RICE GROW UPGRADE SERVER RESPONSE:",
                    data
                );
                ...
                console.error(
                    "Rice grow upgrade request error:",
                    error
                );
    ...
    potatoGrowUpgrade.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                const response = await fetch(
                    "https://server-72ja.onrender.com/api/potato-grow-upgrade",
                    {
                        method: "POST",
                ...
                console.log(
                    "POTATO GROW UPGRADE SERVER RESPONSE:",
                    data
                );
                ...
                console.error(
                    "Potato grow upgrade request error:",
                    error
                );
    ...
    cornGrowUpgrade.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                const response = await fetch(
                    "https://server-72ja.onrender.com/api/corn-grow-upgrade",
                    {
                        method: "POST",
                ...
                console.log(
                    "CORN GROW UPGRADE SERVER RESPONSE:",
                    data
                );
                ...
                console.error(
                    "Corn grow upgrade request error:",
                    error
                );
...
if (riceSoupUpgrade) {
    ...
    riceSoupUpgrade.addEventListener(
        ...
        async function () {
            ...
            riceSoupUpgrade.disabled = true;

            try {

                const response = await fetch(
                    "https://server-72ja.onrender.com/api/rice-soup",
                    {
                        method: "POST",
                ...
                await collectRiceSoupForQuest(1);


            } catch (error) {

                console.error(
                    "Rice Soup request error:",
                    error
                );

            } finally {

                riceSoupUpgrade.disabled =
                    false;
            }
        }
    );
...
if (potatoSoupUpgrade) {
    ...
    potatoSoupUpgrade.addEventListener(
        ...
        async function () {
            ...
            potatoSoupUpgrade.disabled = true;
            ...
            try {
                ...
                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/potato-soup",
                        {
                            method: "POST",
                ...
                await collectPotatoSoupForQuest(1);
                ...
                potatoSoupUpgrade.disabled = false;
    ...
    cornSoupUpgrade.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/corn-soup",
                        {
                            method: "POST",
                ...
                await collectCornSoupForQuest(1);
                ...
                cornSoupUpgrade.disabled = false;
...
if (convertNightLilyButton) {
    ...
    convertNightLilyButton.addEventListener(
        ...
        async function () {
            ...
            convertNightLilyButton.disabled =
                true;
            ...
            try {
                ...
                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/night-lily",
                        {
                            method: "POST",
...
let nightTapUpgradeCost = 10;

/* =========================
   CPS UPGRADE
========================= */

let nightCpsUpgradeCost = 10;

/* =========================
   LOAD LEVELS
========================= */

let nightTapUpgradeLevel = 0;

let nightCpsUpgradeLevel = 0;
...
if (buyNightTapUpgrade) {
    ...
    buyNightTapUpgrade.addEventListener(
        ...
        async function () {
            ...
            buyNightTapUpgrade.disabled = true;
            ...
            try {
                ...
                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/moon-power",
                        {
                            method: "POST",
                ...
                console.log(
                    "NEW MOON POWER LEVEL:",
                    nightTapUpgradeLevel
                );

                console.log(
                    "NEXT MOON POWER COST:",
                    nightTapUpgradeCost
                );
...
if (buyNightCpsUpgrade) {
  ...
  console.log(
            "🌙 NIGHT INCOME BUTTON CLICKED"
        );

        console.log(
            "🌙 NIGHT LILIES BEFORE PURCHASE:",
            nightLilies
        );
        ...
        console.log(
            ...
            buyNightCpsUpgrade.disabled = true;
...
console.log(
    "🌙 SENDING NIGHT INCOME REQUEST"
);

                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/night-income",
                        {
                            method: "POST",
                ...
                nightBonus =
                    Number(data.nightBonus);

                nightCpsUpgradeLevel =
                    Number(
                        data.nightCpsUpgradeLevel
                    );

                nightCpsUpgradeCost =
                    Number(
                        data.nightCpsUpgradeCost
                    );
                ...
                await collectNightIncomeForQuest(1);
                ...
                console.log(
                    "NEW NIGHT INCOME LEVEL:",
                    nightCpsUpgradeLevel
                );

                console.log(
                    "NEW NIGHT INCOME COST:",
                    nightCpsUpgradeCost
                );
                ...
                buyNightCpsUpgrade.disabled = false;
...
/* =========================================================
   FIELD HARVEST TOOLTIP - SAFE VERSION
========================================================= */

const fieldHarvestTooltip =
    document.createElement("div");

fieldHarvestTooltip.className =
    "fieldHarvestTooltip";
...
let hoveredFieldPlot = null;

let harvestTooltipInterval = null;
...
function formatHarvestTime(milliseconds) {
...
function updateFieldHarvestTooltip() {
    ...
    if (!crop) {
        ...
        fieldHarvestTooltip.style.display =
            "none";
...
function startHarvestTooltipTimer() {

    if (harvestTooltipInterval) {
        return;
    }


    harvestTooltipInterval =
        setInterval(
            function () {
...
function stopHarvestTooltipTimer() {
...
function setupFieldHarvestTooltips() {
    ...
    if (
        fieldPlots.dataset.tooltipSetup ===
        "true"
    ) {
...
fieldPlots.addEventListener(
    ...
    function (event) {
        ...
        /* Make sure this plot belongs to the field container */

        if (
            !fieldPlots.contains(plot)
        ) {
            return;
        }

        /* Ignore movement between elements inside same plot */

        if (
            hoveredFieldPlot === plot
        ) {
            return;
        }
    ...
    "block";
        ...
        fieldHarvestTooltip.style.left =
            (
                event.clientX + 12
            ) + "px";

        fieldHarvestTooltip.style.top =
            (
                event.clientY + 12
            ) + "px";
...
fieldPlots.addEventListener(
    ...
    function (event) {
        ...
        /* Ignore movement between children inside same plot */
...
/* =========================================================
   TADS REWARD POPUP
   HARD FORCE DEBUG VERSION
========================================================= */

function showTadsRewardPopup(message) {

    const popup =
        document.getElementById("tadsRewardPopup");

    const messageElement =
        document.getElementById("tadsRewardMessage");

    const closeButton =
        document.getElementById("tadsRewardClose");
    ...
    if (
        ...
        console.error(
            "❌ TADS REWARD POPUP ELEMENTS NOT FOUND"
        );
        return;
    }
    ...
    /* =====================================================
       MAKE SURE POPUP IS DIRECTLY INSIDE BODY
    ===================================================== */
...
const adsUpgradesButton =
    document.getElementById("adsUpgradesButton");

const adsUpgradesOverlay =
    document.getElementById("adsUpgradesOverlay");
 ...
 const onShowRewardCallback =
    async (result) => {
        ...
        try {
            ...
            const response =
                await fetch(
                    "https://server-72ja.onrender.com/api/tads/reward",
            ...
            /* =================================================
               FARMER ACHIEVEMENTS UNLOCKED FROM AD
            ================================================= */
            ...
            /* =================================================
               CPS ACHIEVEMENTS UNLOCKED FROM AD
            ================================================= */
            ...
            /* =================================================
               SHOW GEM ACHIEVEMENTS UNLOCKED
            ================================================= */

            if (
                Array.isArray(
                    data.gemNewlyUnlocked
                )
            ) {

                data.gemNewlyUnlocked.forEach(
                    (achievement) => {
            ...
            /* =================================================
               COMPLETE FARMER DAILY QUEST LOCALLY
            ================================================= */

            if (
                data.farmerQuestCompleted
            ) {
            ...
            if (
                ...
                rewardMessage +=
                    ...
                    data.farmerQuestReward +
                    " GEM\n" +
                    "Farmer daily quest completed!";

            }


          showTadsRewardPopup(
    rewardMessage.replace(
        /\n/g,
        "<br>"
    )
);
...
if (adsUpgradePowerButton) {
    ...
    adsUpgradePowerButton.addEventListener(
        ...
        async () => {
            ...
            if (
                ...
                console.log(
                    "MONETAG: AD UPGRADE IS ON COOLDOWN"
                );
            ...
            if (
                typeof show_11845666 !== "function"
            ) {

                console.error(
                    "MONETAG: REWARDED AD FUNCTION NOT AVAILABLE"
                );
            ...
            console.log(
                "MONETAG: WATCH AD BUTTON CLICKED"
            );
            ...
            try {
                ...
                if (
                    ...
                    console.error(
                        "MONETAG: TELEGRAM INIT DATA NOT AVAILABLE"
                    );
                ...
                console.log(
                    "MONETAG: CLAIMING FARMER REWARD"
                );


                const response =
                    await fetch(
                        "https://server-72ja.onrender.com/api/monetag/reward",
                        {
                ...
                if (
                    typeof data.perSecond !== "undefined"
                ) {
                ...
                if (
                    typeof data.totalGemsEarned !== "undefined"
                ) {
                ...
                if (
                    typeof data.totalGemsBonus !== "undefined"
                ) {
                ...
                if (
                    typeof data.farmerCost !== "undefined"
                ) {
                ...
                if (
                    typeof data.farmers50Achievement !== "undefined"
                ) {

                    farmers50Achievement =
                        data.farmers50Achievement;

                }

                if (
                    typeof data.farmers125Achievement !== "undefined"
                ) {

                    farmers125Achievement =
                        data.farmers125Achievement;

                }

                if (
                    typeof data.farmers180Achievement !== "undefined"
                ) {

                    farmers180Achievement =
                        data.farmers180Achievement;

                }

                if (
                    typeof data.farmers275Achievement !== "undefined"
                ) {

                    farmers275Achievement =
                        data.farmers275Achievement;

                }

                if (
                    typeof data.farmers400Achievement !== "undefined"
                ) {

                    farmers400Achievement =
                        data.farmers400Achievement;

                }

                if (
                    typeof data.farmers600Achievement !== "undefined"
                ) {

                    farmers600Achievement =
                        data.farmers600Achievement;

                }

                if (
                    typeof data.farmers800Achievement !== "undefined"
                ) {

                    farmers800Achievement =
                        data.farmers800Achievement;

                }

                if (
                    typeof data.farmers950Achievement !== "undefined"
                ) {

                    farmers950Achievement =
                        data.farmers950Achievement;

                }

                if (
                    typeof data.farmers1100Achievement !== "undefined"
                ) {

                    farmers1100Achievement =
                        data.farmers1100Achievement;
                ...
                /* =========================================
                   UPDATE CPS ACHIEVEMENT FLAGS
                ========================================= */

                if (
                    typeof data.cps100Achievement !== "undefined"
                ) {
                ...
                if (
                    typeof data.cps500Achievement !== "undefined"
                ) {
                ...
                if (
                    typeof data.cps800Achievement !== "undefined"
                ) {
                ...
                if (
                    typeof data.cps1250Achievement !== "undefined"
                ) {
                ...
                if (
                    typeof data.cps1750Achievement !== "undefined"
                ) {
                ...
                if (
                    typeof data.cps2500Achievement !== "undefined"
                ) {
                ...
                if (
                    typeof data.cps3500Achievement !== "undefined"
                ) {
                ...
                if (
                    typeof data.cps4500Achievement !== "undefined"
                ) {
                ...
                if (
                    typeof data.cps6000Achievement !== "undefined"
                ) {
    ...
    console.error(
        "MONETAG: WATCH AD BUTTON NOT FOUND"
    );

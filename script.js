...
// Base CPS + purchased CPS
const baseCPS =
    (Number(perSecond) || 0) +
    (Number(cpsBonus) || 0);

// Apply ALL percentage bonuses
const finalCPS =
    baseCPS *
    bonusMultiplier;
...
if (coinsPerSecondElement) {
    ...
    const cpsBonusMultiplier =
        1 + (Number(cpsBonus) || 0) / 100;
...
alert(
    ...
    "cpsBonus: " + cpsBonus + "\n" +
    "tonBonus: " + tonBonus + "%\n" +
    "getTotalBonus(): " + getTotalBonus() + "%\n" +
    "Expected CPS: " +
    (
        (Number(perSecond) + Number(cpsBonus)) *
        (1 + Number(getTotalBonus()) / 100)
    )
);
